...
public class HolyStrikeAction : BaseAction
{
    [SerializeField]
    private int _damage;
    ...
    public void Attack()
    {
        ...
        unit.SpawnObjectClientRpc(unit.TargetPosition.Value, 4);
    ...
    {
        int distance = 2;
        _damage = (int)Mathf.Floor(unit.Stats.Value.Strength * 1f);
        ...
        for (int i = -distance; i <= distance; i++)
        {
            for (int j = -distance; j <= distance; j++)
            {
                ...
                if (Mathf.Abs(i) + Mathf.Abs(j) <= distance)
                {
                    ...
                    {
                        currentPlayer.SetMapVisualTileActiveClientRpc(x, y, new Color(1f, 1, 0f, 0.5f));
                    }
                }
            }
        }
